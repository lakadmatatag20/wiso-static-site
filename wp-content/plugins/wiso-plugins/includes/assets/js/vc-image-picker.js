...
if (function($) {
    var wisoResponsiveAlignmentParam = Backbone.View.extend({
        events: {},
    ...
    // VC wiso responsive aligment js
    vc.atts.responsive_alignment = {
        parse: function(param) {
            return this.content().find("input.wpb_vc_param_value." + param.param_name).data("wisoResponsiveAlignment").save().join(" ")
        },
        init: function(param, $field) {
            $('[data-column-offset="true"]', $field).each(function() {
                var $this = $(this);
                $this.find(".wpb_vc_param_value").data("wisoResponsiveAlignment", new wisoResponsiveAlignmentParam({
                    el: $this
                }).render())
            })
        }
...
(function($) {
    ...
    vc.atts.gradient = {
        ...
        init: function(param, $field) {
            ...
            var defaults = {
                ...
                targetInputElement: $field.find('.wiso-gradient-css'),
                targetBgInputElement: $field.find('.wiso-gradient-bg'),
                startingGradient: startingGradient,
                startingBgProps: startingBgProps
            };

            var options = $.extend(true, {}, defaults, param.settings);
            $field.find('.wiso-gradient').icsge(options);
        },

        parse: function(param) {
            var $field = this.content().find(".wpb_vc_param_value[name=" + param.param_name + "]"),
                $css = $field.next('.wiso-gradient-css'),
                $bg = $css.next('.wiso-gradient-bg');

            return $css.val() + '|' + $bg.val();
        }
    };

    // VC wiso slider parameter js
    vc.atts.wiso_slider = {
        ...
        init: function( param, $field ) {
            ...
            var $handle = $field.find('.wiso-handle');

            $field.find('.wiso-slider').slider({
                min: param.min,
                max: param.max,
                step: param.step,
                value: $init_val,
    ...
    vc.atts.wiso_shape_divider = {
        ...
        init: function(param, $field) {
            $(".wiso_color-control", $field).each(function() {
            ...
            $( ".wiso-slider", $field).each( function() {
                var $init_val, $control = $( this ),
                    value  = $control.parent().find( ".wiso-sliderinput").val(),
                    $handle = $control.find('.wiso-handle');
                ...
                $control.slider({
                    ...
                    change: function( event, ui ) {
                        $control.parent().find( ".wiso-sliderinput").val( ui.value );
                    }
                });
            });

            $('h3.wiso-shape-divider-heading', $field).click(function(e) {
                $(this).next('.wiso-inner-wrap').removeClass('hidden')
                    .parent().siblings().children('.wiso-inner-wrap').addClass('hidden');
            });

            $(document).on('change', '.wiso_shape_divider-type', function() {
                ...
                if ( typeof this.options !== typeof undefined ) {
                    ...
                    var selectedIndex = this.options[this.selectedIndex].index,
                        ...
                        previewTarget = $($this.data('preview-target')),
    ...
    vc.atts.responsive_css_editor = {
        ...
        init: function(param, $field) {
            $('h3.wiso-responsive-css-heading', $field).click(function(e) {
                $(this).parent('.wiso-main-responsive-wrapper').addClass('active').siblings().removeClass('active');
            });
        },

    };

    // VC Advanced wiso Checkbox js
    vc.atts.wiso_advanced_checkbox = {

        parse: function(param) {

            if ($(".wiso-advanced-checkbox input").is(':checked')) {
                var $checked = 'checked="checked"';
            } else {
                var $checked = '';
            }

            var $wiso_panel = $('#vc_ui-panel-edit-element'),
                $easy_mode_checkbox = $('<label class="wiso-easy-mode-switch"><input type="checkbox" class="wiso-easy-mode-checkbox" ' + $checked + ' value="true">Easy Mode<span><i class="fa fa-power-off"> </i></span></label>'),
                $easy_mode;

            $wiso_panel.find('.wiso-easy-mode-switch').remove();
            $wiso_panel.find('.vc_ui-panel-header-controls').prepend($easy_mode_checkbox);
            $easy_mode = $easy_mode_checkbox.find('.wiso-easy-mode-checkbox');

            var $advanced_checkbox = $(".wiso-advanced-checkbox input");
    ...
    // run wiso functions after shortcode options changed in frontend editor
    ...
    //
    // 	if ( ev === "update" && typeof wisoMaps !== typeof undefined ) {
    // 		$('[data-plugin-map]').wisoMaps();
    // 	};
    //
    // });
    ...
    // 	if ( ev === "update" && typeof wisoParallax !== typeof undefined ) {
    // 		$('[data-parallax]').wisoParallax();
    // 	};
    //
    // });


    function templates_wiso() {

        if(jQuery('.wiso-templates-container').length){
            jQuery('.wiso-templates-container > .vc_ui-list-bar').each(function () {
                var layoutM = 'masonry';
                jQuery(this).isotope({
                    itemSelector: '.vc_ui-template:not(.hidden)',
                    layoutMode: layoutM,
                    masonry: {
                        columnWidth: '.vc_ui-template:not(.hidden)'
                    }
                });
            });
        }
    ...
    vc.TemplateWindowUIPanelBackendEditor = vc.TemplatesPanelViewBackend.vcExtendUI(vc.HelperPanelViewHeaderFooter).vcExtendUI(vc.HelperTemplatesPanelViewSearch).extend({
        ...
        show: function() {
            ...
            this.clearSearch(), vc.TemplateWindowUIPanelBackendEditor.__super__.show.call(this), $('.vc_edit-form-tab[data-tab="wiso_templates"] .sortable_templates ul > li').each(function() {
                "all" == $(this).attr("data-sort") ? $(this).find(".count").html($('.vc_edit-form-tab[data-tab="wiso_templates"] .vc_ui-template-list > .vc_ui-template').length/2) : $(this).find(".count").html($('.vc_edit-form-tab[data-tab="wiso_templates"] .vc_ui-template-list > .vc_ui-template.' + $(this).attr("data-sort")).length/2);

            }), $('.vc_edit-form-tab[data-tab="wiso_templates"] .sortable_templates li[data-sort="all"]').addClass("active").trigger("click"), $('.vc_edit-form-tab[data-tab="wiso_templates"] .sortable_templates li').click(function() {
                $('.vc_edit-form-tab[data-tab="wiso_templates"] .sortable_templates li').removeClass("active"), $(this).addClass("active");
                var t = $(this).attr("data-sort");
                $('.vc_edit-form-tab[data-tab="wiso_templates"] .vc_ui-template-list > .vc_ui-template').removeClass("hidden"), "all" != t && $('.vc_edit-form-tab[data-tab="wiso_templates"] .vc_ui-template-list > .vc_ui-template:not(.' + t + ")").addClass("hidden")

                setTimeout(templates_wiso,0);
            }),
            ...
            //
            // jQuery('.wiso-templates-container img').on('load', function () {
            //
            //     alert('hi');
        ...
        showMessageDisabled: !1,
        show: function() {
            this.clearSearch(), vc.TemplateWindowUIPanelFrontendEditor.__super__.show.call(this), $('.vc_edit-form-tab[data-tab="wiso_templates"] .sortable_templates ul > li').each(function() {
                "all" == $(this).attr("data-sort") ? $(this).find(".count").html($('.vc_edit-form-tab[data-tab="wiso_templates"] .vc_ui-template-list > .vc_ui-template').length/2) : $(this).find(".count").html($('.vc_edit-form-tab[data-tab="wiso_templates"] .vc_ui-template-list > .vc_ui-template.' + $(this).attr("data-sort")).length/2)
            }), $('.vc_edit-form-tab[data-tab="wiso_templates"] .sortable_templates li[data-sort="all"]').addClass("active").trigger("click"), $('.vc_edit-form-tab[data-tab="wiso_templates"] .sortable_templates li').click(function() {
                $('.vc_edit-form-tab[data-tab="wiso_templates"] .sortable_templates li').removeClass("active"), $(this).addClass("active");
                var t = $(this).attr("data-sort");
